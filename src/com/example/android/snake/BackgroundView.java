...
public class BackgroundView extends View {
	...
	public BackgroundView(Context context, AttributeSet attrs) {
		...
		mColors[0] = a.getColor(R.styleable.BackgroundView_colorSegmentOne,
				Color.RED);
		mColors[1] = a.getColor(R.styleable.BackgroundView_colorSegmentTwo,
				Color.YELLOW);
		mColors[2] = a.getColor(R.styleable.BackgroundView_colorSegmentThree,
				Color.BLUE);
		mColors[3] = a.getColor(R.styleable.BackgroundView_colorSegmentFour,
				Color.GREEN);
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int triangle = 0; triangle < mColors.length; triangle++) {
			...
			canvas.drawVertices(Canvas.VertexMode.TRIANGLES,
					mVertexPoints.length, mVertexPoints, 0, null, 0, // No
																		// Textures
					mFillColors, 0, mIndices, triangle * 2, 3, // Use 3 vertices
																// via Index
																// Array with
																// offset 2
					new Paint());
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		mVertexPoints = new float[] { w / 2, h / 2, 0, 0, w, 0, w, h, 0, h };
	}
